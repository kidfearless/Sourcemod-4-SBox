...
namespace Sourcemod
{
	public partial class SourceMod
	{
		 ...
		 */
		public static int strcopy(out string dest, int destLen, string source) { throw new NotImplementedException(); }
		 ...
		 * @return              Floating point result, or 0.0f on error.
		 */
		...
		public static int StrCat(string buffer, int maxlength, string source)
		{
			...
			return Format(buffer.Substring(len), maxlength - len, "%s", source);
		}
							...
							int maxStringLength, bool copyRemainder = false)
		{
			...
			while ((idx = SplitString(text.Substring(reloc_idx), split, buffers[total], maxStringLength)) != -1)
			{
				reloc_idx += idx;
				if (++total == maxStrings)
				{
					if (copyRemainder)
					{
						strcopy(out buffers[total - 1], maxStringLength, text.Substring(reloc_idx - idx));
					}
					return total;
				}
			}

			strcopy(out buffers[total++], maxStringLength, text.Substring(reloc_idx));
